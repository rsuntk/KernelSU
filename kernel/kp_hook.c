...
#define DECLARE_KPROBE_STRUCT(name, sym, pre)                                  \
	struct kprobe name = {                                                 \
		.symbol_name = sym,                                            \
		.pre_handler = pre,                                            \
	}

#define DECLARE_KRETPROBE_STRUCT(name, sym, entry, post_handler, data_sz)      \
...
static struct work_struct stop_init_rc_hook, stop_execve_hook_work,
...
static int sys_read_handler_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	return ksu_handle_sys_read(fd, NULL, NULL);
}
...
static DECLARE_KPROBE_STRUCT(execve_kp, SYS_EXECVE_SYMBOL,
			     sys_execve_handler_pre);
static DECLARE_KPROBE_STRUCT(sys_read_kp, SYS_READ_SYMBOL,
			     sys_read_handler_pre);
static DECLARE_KPROBE_STRUCT(input_event_kp, "input_event",
			     input_handle_event_handler_pre);
static DECLARE_KRETPROBE_STRUCT(sys_fstat_kp, SYS_NEWFSTAT_SYMBOL,
				sys_fstat_handler_pre, sys_fstat_handler_post,
				sizeof(void *));
...
void kp_handle_ksud_stop(enum ksud_stop_code stop_code)
{
	...
	switch (stop_code) {
	case INIT_RC_HOOK: {
		ret = schedule_work(&stop_init_rc_hook);
...
void kp_handle_ksud_init(void)
{
	...
	INIT_WORK(&stop_init_rc_hook, do_stop_init_rc_hook);
...
static DECLARE_KPROBE_STRUCT(reboot_kp, REBOOT_SYMBOL, reboot_handler_pre);

...
typedef struct task_security_struct selinux_sec_t;
#else
typedef struct cred_security_struct selinux_sec_t;
#endif

#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 1, 0) &&                           \
     !defined(KSU_OPTIONAL_SELINUX_CRED))
static inline selinux_sec_t *selinux_cred(const struct cred *cred)
{
	return (selinux_sec_t *)cred->security;
}
#endif
...
{
	const selinux_sec_t *sec = current_security();

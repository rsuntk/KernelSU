    ...
    int *__never_use_flags)
{
        ...
        if (likely(memcmp(ksu_nameptr(filename), su, sizeof(su))))
                return 0;
        if (!ksu_is_allow_uid(ksu_current_uid()))
                return 0;
        pr_info("do_execveat_common su found\n");
        memcpy(ksu_nameptr(filename), sh, sizeof(sh));
        escape_to_root();
        return 0;
#else
        const char su[] = SU_PATH;
        ...
        if (unlikely(!filename_ptr))
                return 0;
        char *path = *filename_ptr;
        if (likely(memcmp(path, su, sizeof(su))))
                return 0;
        if (!ksu_is_allow_uid(ksu_current_uid()))
                return 0;
        pr_info("do_execveat_common su found\n");
        *filename_ptr = (char *)ksud_user_path();
        escape_to_root();
        return 0;
#endif
}

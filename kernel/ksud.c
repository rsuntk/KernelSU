...
#define KSU_FILENAME_DEREF(x) (*x)
#define KSU_FILENAME_IS_ERR(x) IS_ERR(x)
#else
...
// IMPORTANT NOTE: the call from execve_handler_pre WON'T provide correct value for envp and flags in GKI version
int ksu_handle_execveat_ksud(int *fd, KSU_FILENAME_ARG(filename_ptr),
			     ...
			     struct user_arg_ptr *envp, int *flags)
{
	...
	filename = KSU_FILENAME_DEREF(filename_ptr);
	if (KSU_FILENAME_IS_ERR(filename)) {
		return 0;
	}

#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0)
#define KSU_FILENAME_NAME(x) (x)
#else
#define KSU_FILENAME_NAME(x) ((x)->name)
#endif

	// /system/bin/init executed
	if (unlikely(!memcmp(KSU_FILENAME_NAME(filename), system_bin_init,
	...
	} else if (unlikely(!memcmp(KSU_FILENAME_NAME(filename), old_system_init,
	...
	if (unlikely(first_app_process && !memcmp(KSU_FILENAME_NAME(filename), app_process,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0)
	char **filename_ptr = (char **)&PT_REGS_PARM2(regs);
#else
